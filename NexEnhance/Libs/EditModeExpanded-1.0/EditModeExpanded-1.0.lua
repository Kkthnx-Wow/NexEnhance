...
function lib:RegisterFrame(frame, name, db, anchorTo, anchorPoint, clamped)
	...
	frame.Selection:SetGetLabelTextFunction(function()
		return name
	end)
	...
	if db.clamped == nil then
		db.clamped = (clamped and 1 or 0)
	end
	...
	if not framesDialogs[frame.system] then
		framesDialogs[frame.system] = {}
	end
	if not framesDialogsKeys[frame.system] then
		framesDialogsKeys[frame.system] = {}
	end
	...
	resetButton:SetScript("OnClick", function()
		...
		if not db.defaultX then
			db.defaultX = 0
		end
		if not db.defaultY then
			db.defaultY = 0
		end
	...
	EditModeManagerExpandedFrame:HookScript("OnShow", function()
		if resetButton.hiddenByGrouping then
			return
		end
		resetButton:Show()
	end)
...
function lib:RegisterResizable(frame)
	...
	if not framesDialogs[systemID] then
		framesDialogs[systemID] = {}
	end
	if framesDialogsKeys[systemID] and framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_FRAMESIZE] then
		return
	end
	if not framesDialogsKeys[systemID] then
		framesDialogsKeys[systemID] = {}
	end
...
function lib:RegisterHideable(frame, onEventHandler)
	...
	if framesDialogsKeys[systemID] and framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_HIDEABLE] then
		return
	end
	if not framesDialogsKeys[systemID] then
		framesDialogsKeys[systemID] = {}
...
function lib:IsFrameMarkedHidden(frame)
	...
	if not framesDB[systemID].settings then
		framesDB[systemID].settings = {}
	end
...
function lib:RegisterCustomCheckbox(frame, name, onChecked, onUnchecked, internalName)
	...
	if not framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_CUSTOM] then
		framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_CUSTOM] = {}
	end
	...
	local function callLater()
		...
		if not db.settings[ENUM_EDITMODEACTIONBARSETTING_CUSTOM] then
...
function lib:RegisterDropdown(frame, libUIDropDownMenu, internalName)
	...
	if not framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_DROPDOWN] then
		framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_DROPDOWN] = {}
	end
	...
	local function getCurrentDB()
		...
		if not db.settings[ENUM_EDITMODEACTIONBARSETTING_DROPDOWN] then
			db.settings[ENUM_EDITMODEACTIONBARSETTING_DROPDOWN] = {}
		end
		if not db.settings[ENUM_EDITMODEACTIONBARSETTING_DROPDOWN][internalName] then
			db.settings[ENUM_EDITMODEACTIONBARSETTING_DROPDOWN][internalName] = {}
		end
...
function lib:RegisterSlider(frame, name, internalName, onChanged, min, max, step)
	...
	if not framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_SLIDER] then
		framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_SLIDER] = {}
	end
	...
	local function callLater()
		...
		if not db.settings[ENUM_EDITMODEACTIONBARSETTING_SLIDER] then
			db.settings[ENUM_EDITMODEACTIONBARSETTING_SLIDER] = {}
		end
...
hooksecurefunc(f, "OnLoad", function()
	...
	hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function(self)
		...
		if not EditModeManagerExpandedFrame then
			return
		end -- happens if library is embedded but nothing has been registered
	...
	frame.Buttons.Divider:SetMask("InterfaceFriendsFrameUI-FriendsFrame-OnlineDivider")
				...
				for index, displayInfo in ipairs(systemSettingDisplayInfo) do
					...
					if settingFrame then
						...
						if displayInfo.setting == ENUM_EDITMODEACTIONBARSETTING_SLIDER then
							if not framesDB[systemID].settings[displayInfo.setting] then
								framesDB[systemID].settings[displayInfo.setting] = {}
							end
	...
	function EditModeExpandedSystemSettingsDialog:OnSettingValueChanged(setting, value, internalName, onChanged)
		...
		if attachedToSystem then
			...
			elseif setting == ENUM_EDITMODEACTIONBARSETTING_SLIDER then
				if not db.settings[setting] then
					db.settings[setting] = {}
				end
			...
			if new_x ~= x or new_y ~= y then
				...
				if existingFrames[frame:GetName()] then
					...
					for index, frameData in ipairs(activeLayout.systems) do
						...
						if (anchorInfo.point == a) and (anchorInfo.relativeTo == b:GetName()) and (anchorInfo.relativePoint == c) and (tonumber(string.format("%.3f", anchorInfo.offsetX)) == tonumber(string.format("%.3f", d))) and (tonumber(string.format("%.3f", anchorInfo.offsetY)) == tonumber(string.format("%.3f", e))) then
...
function lib:RegisterToggleInCombat(frame)
	...
	if framesDialogsKeys[systemID] and framesDialogsKeys[systemID][ENUM_EDITMODEACTIONBARSETTING_TOGGLEHIDEINCOMBAT] then
		return
	end
	if not framesDialogsKeys[systemID] then
		framesDialogsKeys[systemID] = {}
