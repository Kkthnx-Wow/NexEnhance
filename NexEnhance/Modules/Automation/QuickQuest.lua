...
local function GetNPCID()
	return Module.GetNPCID(UnitGUID("npc"))
end
...
-- Handle ignore list
function Module:UpdateIgnoreList()
...
local function UnitQuickQuestStatus(self)
	if not self.__ignore then
		...
		frame:SetPoint("TOP", self, "BOTTOM", 0, -4)
		Module.AddTooltip(frame, "ANCHOR_RIGHT", "|nThis NPC is now ignored for auto-quest interactions.|n|nHold the ALT key and click the name above to remove it from the ignore list.", "info", "Ignored NPC", false)
		Module.CreateFontString(frame, 14, IGNORED):SetTextColor(1, 0, 0)

		self.__ignore = frame

		Module:UpdateIgnoreList()
	end
