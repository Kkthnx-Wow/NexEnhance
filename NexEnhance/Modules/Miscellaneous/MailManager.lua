...
function Module:MailBox_DelectClick()
	...
	if InboxItemCanDelete(selectedID) then
		...
		UIErrorsFrame:AddMessage(Module.InfoColor .. ERR_MAIL_DELETE_ITEM_ERROR)
	end
end
...
function Module:InboxItem_OnEnter()
	...
	if itemAttached then
		...
		if itemAttached > 1 then
			GameTooltip:AddLine(Module.L["Attach List"])
...
function Module:TotalCash_OnEnter()
	...
	if totalCash > 0 then
		...
		GameTooltip:AddLine(Module.L["Total Gold"])
		GameTooltip:AddLine(" ")
		GameTooltip:AddLine(Module:GetMoneyString(totalCash, true), 1, 1, 1)
		GameTooltip:Show()
	end
end
...
function Module:MailBox_CollectCurrent()
	if OpenMailFrame.cod then
		UIErrorsFrame:AddMessage(Module.InfoColor .. Module.L["Mail Is COD"])
		return
	end
...
function Module:CollectCurrentButton()
	local button = Module:MailBox_CreatButton(OpenMailFrame, 82, 22, Module.L["Take All"], { "RIGHT", "OpenMailReplyButton", "LEFT", -1, 0 })
...
function Module:ArrangeDefaultElements()
	...
	SendMailMailButton:HookScript("OnEnter", function(self)
		...
		GameTooltip:AddLine(SEND_MAIL_COST .. colorStr .. Module:GetMoneyString(sendPrice, true))
		GameTooltip:Show()
	end)

	SendMailMailButton:HookScript("OnLeave", GameTooltip.Hide)
end
