...
function Module:AddLineForID(id, linkType, noadd)
	...
	if linkType == types.item then
		...
		if bankCount > 0 then
			self:AddDoubleLine(BAGSLOT .. "/" .. BANK .. ":", Module.InfoColor .. bagCount .. "/" .. bankCount)
		elseif bagCount > 0 then
			self:AddDoubleLine(BAGSLOT .. ":", Module.InfoColor .. bagCount)
		end
		if itemStackCount and itemStackCount > 1 then
			self:AddDoubleLine(Module.L["Stack Cap"] .. ":", Module.InfoColor .. itemStackCount)
		end
	end

	self:AddDoubleLine(linkType, format(Module.InfoColor .. "%s|r", id))
	self:Show()
end
...
function Module:PLAYER_LOGIN()
	...
	hooksecurefunc(GameTooltip, "SetUnitAura", function(self, ...)
		...
		if caster then
			...
			local hexColor = Module.RGBToHex(Module.UnitColor(caster))
			self:AddDoubleLine(Module.L["From"] .. ":", hexColor .. name)
			self:Show()
		end
	end)
